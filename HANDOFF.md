...
## 40. 最新交接补充（2026-07-13 - 诊断广播用例、OTA A1 参数与青桔 SN 校验修复）

本轮根据实测反馈继续收口美团测试执行与青桔产线检测问题，重点保证“执行动作、证据日志、自动判定”三者一致：

- **美团诊断广播用例同步修正**：
  - `MT-RFID-DIAG-004` 禁用周期广播后，自动发送 `SID=0x02` 重启请求模拟下电再上电，判定重启后 `0x2C0~0x2C6` 广播恢复。
  - `MT-RFID-DIAG-005` 自动执行改为先发送禁用广播 `02 28 00`，再发送使能广播 `02 28 01`，判定要求先后采集到对应肯定响应并确认广播恢复。
  - `MT-RFID-DIAG-006/007` 覆盖 `0x2C0~0x2C6` 每一帧：自动逐条下发 `0x29` 周期配置，随后发送 `SID=0x02` 重启，判定每条肯定响应、配置效果以及重启后的保持状态；`0xFFFF` 场景要求重启后仍保持禁止广播。
  - 新增 `mt.sid_0x29_restore_2c0_to_2c6_default` 后置恢复模板，执行完成后将 `0x2C0~0x2C6` 恢复为 `0x0064`。

- **美团 OTA BOOT 恢复场景手动 A1 参数**：
  - OTA 页新增“BOOT恢复A1参数”输入区，可在终端停留 BOOT、无法从 0x2C3 自动获取厂商代码和硬件版本时，手动输入 A1 使用的厂商代码与硬件版本。
  - 输入支持十六进制格式，厂商代码按 1 字节校验，硬件版本按 2 字节 BCD 校验，范围限制为 `0000~0999`，避免非法 A1 参数直接进入升级流程。

- **青桔产线 SN 写入/回读校验一致性修复**：
  - 修复 25 位青桔二维码 SN 写入已取后 11 位但回读校验仍按旧规则“取后 10 位并补 0”的问题。
  - 新增 `qingjuProductionSnPayload()` 统一生成 A00D 实际写入值与回读期望值：25 位 SN 直接取后 11 位，不再补 `0`；旧格式仍保留原兼容逻辑。

- **压力测试 TAG 统计补强**：
  - 青桔/美团压力测试侧继续使用占位 TAG 过滤和 16 字节 TAG 扩展占位处理，避免全 `0x30` 占位值被计为有效 TAG。
...
### 最新测试状态
- JSON 用例资源校验：PASS（`ConvertFrom-Json` 成功）
- Release 构建：PASS（2026-07-09，在 `F:\TestTools\MT_CAN\CAN_RFID_Qt\release` 使用 Qt 5.15.2 MinGW 构建通过）
- 最新可执行文件：`F:\TestTools\MT_CAN\CAN_RFID_Qt\release\CAN_RFID.exe`，更新时间 `2026-07-09 17:14:07`

### 剩余风险与建议
- `0x29` 后置恢复当前统一恢复为 `0x0064`，如果协议最终确认 `0x2C0~0x2C6` 默认周期并非完全一致，应改为按 CAN ID 的默认周期表恢复。
- `MT-RFID-DIAG-006/007` 的重启保持判定依赖采集窗口内能覆盖足够重启后样本，仍建议连接真实终端回归一次完整证据日志。
- 当前工作区仍存在协议 PDF 删除标记和若干未跟踪文档/输出文件，本次提交刻意排除，避免误提交非代码交付内容。
